...
import { ExerciseType, Machine, MachineService } from '@/support'
...
export async function deleteSelected(ids: number[]) {
  for (const id of ids) {
    try {
      ...
      values.value = values.value.filter(
        (x) => x.MachineId !== responseData.DeletedId
      )
    } catch (err) {
      toast({
        title: 'Error while deleting data',
