...
const createCategory = async (values: Record<string, any>) => {
    try{ 
        await new ExerciseCategoryService().Create({ CategoryName: values["CategoryName"] })
    } catch (err) {
        throw err
    }
};
...
const updateCategory = async (values: Record<string, any>) => {
    try{ 
        await new ExerciseCategoryService().Update(values, category.value.CategoryId)
    } catch (err) {
        throw err
    } };
...
<template>
        <AutoForm class="w-2/3 space-y-6" :form="form" :schema="categorySchema" @submit="handleSubmit">
            <Button type="submit">
                Submit
            </Button>
        </AutoForm>
