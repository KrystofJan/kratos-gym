...
<template>
  ...
  <SignedIn>
    <SidebarProvider
      v-if="currentAccount && currentAccount.Role === UserRoleOptions.EMPLOYEE"
    >
      <Sidebar>
        <SidebarHeader>
          <SidebarMenu>
          ...
          <SidebarGroup v-for="item in data.navMain" :key="item.title">
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                ...
                >
                  <SidebarMenuButton
                    :class="{ 'text-slate-50': subItem.isActive }"
                    as-child
                  >
                    <router-link
                      :to="item.url + subItem.url"
                      @click="switchLink(subItem)"
                    >
                      {{ subItem.title }}
                    </router-link>
                  </SidebarMenuButton>
    ...
    <Alert v-else>
      <Terminal class="h-4 w-4" />
      <AlertTitle>Error</AlertTitle>
      <AlertDescription> You need to be an employee </AlertDescription>
    </Alert>
    <div class="ErrorMessage ErrorMessage--notLogedIn MarginedComponent"></div>
  </SignedIn>
</template>
