...
export class MachineService {
  ...
  async SuggestTime(
    ...
    try {
      const res = await fetch(
        `${KRATOS_API_URL}/api/machine/usage/${id}?desired_date=${format(desired_date, 'yyyy-MM-dd')}&desired_start_time=${desired_start_time.hour}:${desired_start_time.minute}&desired_end_time=${desired_end_time.hour}:${desired_end_time.minute}&amount_of_people=${amount_of_people}&can_disturb=${can_disturb}`
      )
      ...
      const data = await res.json()
      const prevSugg: [Time, Time] = [
        data.PrevSuggestion.StartTime,
        data.PrevSuggestion.EndTime,
      ]
      const nextSugg: [Time, Time] = [
        data.NextSuggestion.StartTime,
        data.NextSuggestion.EndTime,
      ]
  ...
  async UpdateMachine(machine: Partial<Machine>, machineId: number) {
    ...
    try {
      const res = await fetch(`${KRATOS_API_URL}/api/machine/${machineId}/`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Accept: 'application/json',
