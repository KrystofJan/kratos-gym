...
export class PlanService extends BaseService<Plan, PlanPost> {
  ...
  async FetchPlansOnDate(params: {
    ...
    try {
      const res = await fetch(
        `${KRATOS_API_URL}/api/${this.path}?machine_id=${machine_id}&date=${format(date, 'yyyy-MM-dd')}`
      )
      if (!res.ok) {
  ...
  async FetchPlansMachines(planId: number): Promise<Machine[]> {
    try {
      const res = await fetch(
        `${KRATOS_API_URL}/api/${this.path}/${planId}/machines`
      )
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`)
      }
