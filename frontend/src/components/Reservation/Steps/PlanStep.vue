...
const fetchData = async () => {
  try {
    const data = await new AccountService().fetchAccountByRole(
      UserRoleOptions.TRAINER
    )
    trainers.value = data
  } catch (error) {
    ...
    >
      ...
      <FormField name="arrivalDate">
        <FormItem class="flex flex-col">
          ...
          <Popover>
            <PopoverTrigger as-child>
              <FormControl>
                ...
                >
                  <span>{{
                    val ? format(val.toString(), 'PPP') : 'Pick a date'
                  }}</span>
          ...
          <FormDescription> Date of arival </FormDescription>
          <FormMessage />
        </FormItem>
      </FormField>
          ...
          <Popover>
            <PopoverTrigger as-child>
              <FormControl>
                <Button
                  ...
                  :class="
                    cn(
                      ...
                      !selectedTrainer ? 'text-muted-foreground' : ''
                    )
                  "
                >
                  {{
                ...
                <span
                  @click="removeTrainer"
                  class="inline-block hover:text-muted-foreground transition h-4 pl-1"
                  :class="
                    cn(
                      'justify-between',
                      !selectedTrainer
                    ...
                    >
                      <Check
                        :class="
                          cn(
                            'mr-2 h-4 w-4',
                            trainer.AccountId === selectedTrainer?.AccountId
                              ? 'opacity-100'
                              : 'opacity-0'
                          )
                        "
