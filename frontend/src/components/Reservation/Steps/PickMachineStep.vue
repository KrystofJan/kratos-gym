...
const { value: selectedMachines, handleChange } =
  useField<Machine[]>('machines')
...
function assignCategories(value: Machine[]) {
  const result = value.flatMap((machine: Machine) =>
    machine.ExerciseTypes.map((type: ExerciseType) => type.Category.CategoryId)
  )
...
const mostFrequentCategoryId = computed(() => {
  ...
  for (const catId of selectedCategories.value) {
    const amount = selectedCategories.value.filter(
      (cat) => cat === catId
    ).length
    if (amount >= max) {
      max = amount
...
watch(
  ...
  async (newId) => {
    ...
    const machineId = machineArray.value.filter((machine: Machine) => {
      return (
        machine.ExerciseTypes.filter((type: ExerciseType) => {
    ...
    try {
      ...
      recommendMachines.value = data
        .filter((machine: Machine) => !machineids.includes(machine.MachineId))
        .slice(0, 5)
    } catch (error) {
    ...
    <form class="w-full flex flex-col gap-4" @submit="onSubmit">
      <FormField name="machines">
        <div class="grid fieldGrid">
          <div
            v-for="(item, index) in machineArray"
        ...
        <div class="grid fieldGrid w-full">
          <div
            v-for="(item, index) in recommendMachines"
            :key="item.MachineId ?? index"
          >
