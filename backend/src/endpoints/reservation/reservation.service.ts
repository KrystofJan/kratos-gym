...
export class ReservationService {
    static async GetAllReservationes(limit?: number, page?: number): Promise<Array<Reservation>> {
        ...
        try {
            const models = databaseResponse.Body.map(
                (model: Reservation) => new Reservation(model)
            )
            return models
        } catch (err) {
            logger.error(err)
            throw new CodedError(
                ...
                'Mapping model at GetAllReservationes failed'
            )
        }
    }
    ...
    static async GetReservationById(id: number): Promise<Reservation> {
        ...
        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectSpecific(Reservation, id)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ...
                `Reservation with an id: '${id}' was not found`
            )
        }
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at GetReservationById failed'
            )
            throw err
        }
        return model
    ...
    static async UpdateReservationById(
        id: number,
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Reservation with an id: '${id}' was not Deleted`
            )
        }
    ...
    static async CreateReservation(body: Reservation): Promise<number> {
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at CreateReservation failed'
            )
            throw err
        }
    ...
    static async GetReservationByAccountId(id: number): Promise<Reservation[]> {
        ...
        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectAttrIs(Reservation, id, 'customer_id')
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
