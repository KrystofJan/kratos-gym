...
PlanRouter.post(
    ...
    async (req: Request, res: Response) => {
        ...
        const startTime: Time = { hour: 9, minute: 0 }
        ...
        for (let i = 0; i < collisions_dataset.length; ++i) {
            ...
            while (j < collisions_dataset[i].length) {
                const { index, start_time, end_time } = func(collisions_dataset[i], j);
                esketit.push({
        ...
        function func(data_tmp: Data[], index: number): { index: number, start_time: Time, end_time: Time } {
        ...
        // if (colliding) {
        // res.status(StatusCodes.OK).json(collisions_data)
        // } else {
        //     res.status(StatusCodes.OK).json(no_collisions_data)
        // }
    })
