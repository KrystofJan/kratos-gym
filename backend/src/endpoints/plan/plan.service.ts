...
export class PlanService {
    ...
    static async GetPlanById(id: number): Promise<Plan> {
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ...
                `Plan with an id: '${id}' was not found`
            )
        }
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at GetPlanById failed'
            )
            throw err
        }
        return model
    ...
    static async UpdatePlanById(
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Plan with an id: '${id}' was not Deleted`
            )
        }
    ...
    static async CreatePlan(body: Plan): Promise<number> {
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at CreatePlan failed'
            )
            throw err
        }
    ...
    static async GetMachinesInPlan(id: number): Promise<MachinesInPlan[]> {
        ...
        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectAttrIs(MachinesInPlan, id, 'plan_id')
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        ...
        try {
            if (databaseResponse instanceof DatabaseFoundMultiple) {
                const models = databaseResponse.Body.map(
                    (model: MachinesInPlan) => new MachinesInPlan(model)
                )
                return models
            }
    ...
    static async AddMachine(body: MachinesInPlan): Promise<MachinesInPlan> {
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Machine with an id: '${body.MachineId}' was not Added`
            )
        }
    ...
    static async AddExerciseType(
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Plan with an id: '${typeId}' was not Added`
            )
        }
    ...
    static async DeleteMachineRecordFromPlan(
        machineId: number,
        planId: number
    ) {
        const db = new BasicQueryDatabase()
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Machine ${machineId} was not deleted from plan '${planId}' `
            )
        }
    ...
    static async DeleteTypeRecordFromPlan(typeId: number, planId: number) {
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Type ${typeId} was not deleted from plan '${planId}' `
            )
        }
    ...
    static async UpdateMachineInPlan(
        planId: number,
        machineId: number,
        body: Partial<MachinesInPlan>
    ): Promise<MachinesInPlan> {
        const db = new BasicQueryDatabase()

        const [databaseErr, databaseResponse] = await safeAwait(
            db.Update(MachinesInPlan, planId, body, 'machine_id', machineId)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Plan with an id: '${machineId}' was not Deleted`
            )
        }
    ...
    static async GetPlansOnDate(id: number, date: Date): Promise<Array<Plan>> {
        ...
        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectMachinesUsedOnDate(id, date)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
