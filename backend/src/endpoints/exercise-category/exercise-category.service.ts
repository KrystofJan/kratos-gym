...
export class ExerciseCategoryService {
    static async GetAllExerciseCategories(): Promise<Array<ExerciseCategory>> {
        ...
        try {
            const models = databaseResponse.Body.map(
                (model: ExerciseCategory) => new ExerciseCategory(model)
            )
            return models
        } catch (err) {
            logger.error(err)
    ...
    static async GetExerciseCategoryById(
        id: number
    ): Promise<ExerciseCategory> {
        const db = new BasicQueryDatabase()

        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectSpecific(ExerciseCategory, id)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
    ...
    static async CreateExerciseCategory(
        body: ExerciseCategory
    ): Promise<number> {
        const db = new BasicQueryDatabase()

        const [databaseErr, databaseResponse] = await safeAwait(
            db.Insert(ExerciseCategory, body)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at CreateExerciseCategory failed'
            )
            throw err
        }
    ...
    static async UpdateExerciseCategoryById(
        id: number,
        body: Partial<ExerciseCategory>
    ): Promise<ExerciseCategory> {
        const db = new BasicQueryDatabase()

        const [databaseErr, databaseResponse] = await safeAwait(
            db.Update(ExerciseCategory, id, body)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `ExerciseCategory with an id: '${id}' was not Deleted`
            )
        }
    ...
    static async GetCategorysByMachineId(
        id: number
    ): Promise<ExerciseCategory[]> {
        const db = new BasicQueryDatabase()
    ...
    static async GetCategoriesByPlanId(
        id: number
    ): Promise<ExerciseCategory[]> {
        const db = new BasicQueryDatabase()
