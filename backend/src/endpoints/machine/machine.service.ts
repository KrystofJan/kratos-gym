...
export class MachineService {
    static async GetAllMachines(
        ...
        try {
            const models = databaseResponse.Body.map(
                (model: Machine) => new Machine(model)
            )
            return models
        } catch (err) {
            logger.error(err)
            throw new CodedError(
                ...
                'Mapping model at GetAllMachinees failed'
            )
        }
    }
    ...
    static async GetRecommendedMachines(id: number): Promise<Array<Machine>> {
        ...
        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectMachinesWithinTheSameCategory(id)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
    ...
    static async GetMachineById(id: number): Promise<Machine> {
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ...
                `Machine with an id: '${id}' was not found`
            )
        }
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at GetMachineById failed'
            )
            throw err
        }
        return model
    ...
    static async UpdateMachineById(
        id: number,
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Machine with an id: '${id}' was not Deleted`
            )
        }
    ...
    static async AddExerciseType(
        ...
        if (databaseResponse.Body === undefined) {
            throw new CodedError(
                ErrorCode.NOT_FOUND_ERROR,
                `Machine with an id: '${typeId}' was not Added`
            )
        }
    ...
    static async CreateMachine(body: Machine): Promise<number> {
        ...
        if (!model) {
            const err = new CodedError(
                ErrorCode.MAPPING_ERROR,
                'Mapping model at CreateMachine failed'
            )
            throw err
        }
    ...
    static async GetMachineUsageByDate(
        id: number,
        date: Date,
        amount_of_people: number
    ): Promise<Array<MachineUsage>> {
        const db = new MachineDatabase()

        const [databaseErr, databaseResponse] = await safeAwait(
            db.SelectMachineUsageByDate(id, date, amount_of_people)
        )
        if (databaseErr !== null) {
            throw databaseErr
        }
        ...
        try {
            const models = databaseResponse.Body.map(
                (model: MachineUsage) => new MachineUsage(model)
            )
            return models
        } catch (err) {
            logger.error(err)
    ...
    static async SuggestTimes(
        ...
        const colidingPlan = machineUsage
            ...
            .filter((time) => {
                const startTime = time.StartTime.split(':').map((x) =>
                    Number(x)
                )
                ...
                const minEndTime =
                    endInt <= desiredEndInt ? endInt : desiredEndInt
                const maxStartTime =
                    startInt >= desiredStartInt ? startInt : desiredStartInt
        ...
        const determineClosestTime = (
            ...
            const prevEndInt = item.Prev
                ? parseTimeToMinutes(item.Prev.EndTime)
                : null
            const nextStartInt = item.Next
                ? parseTimeToMinutes(item.Next.StartTime)
                : null

            let prevSuggestion = null
            let nextSuggestion = null
            let realPrev: TimeRange = {
                StartTime: new Time(),
                EndTime: new Time(),
                isColiding: false,
            }
