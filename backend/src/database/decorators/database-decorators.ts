...
export function Column(name: string) {
    return function (target: Model, propertyKey: string) {
        const columns =
            Reflect.getMetadata(DecoratorType.COLUMN_NAME, target) || []
        columns.push(name)
        ...
        const fieldMap =
            Reflect.getMetadata(DecoratorType.FIELD_MAP, target) || {}
...
export function ForeignKey(type: any) {
    return function (target: Model, propertyKey: string) {
        const fkMap =
            Reflect.getMetadata(DecoratorType.FOREIGN_KEY_MAP, target) || {}
        const columnMap =
            Reflect.getMetadata(DecoratorType.COLUMN_MAP, target) || []
        ...
        const foreignKeyString =
            Reflect.getMetadata(DecoratorType.FOREIGN_KEYS_KEYS, target) || []
        ...
        // "Address": "AddressId"
        const foreignObjectMap =
            Reflect.getMetadata(
                DecoratorType.FOREIGN_PRIMARY_OBJECT_KEY_MAP,
                target
            ) || {}
...
export function UnInsertable() {
    return function (target: Model, propertyKey: string) {
        const unInsertable =
            Reflect.getMetadata(DecoratorType.UNINSERTABLE, target) || []
...
export function UnUpdatable() {
    return function (target: Model, propertyKey: string) {
        const unInsertable =
            Reflect.getMetadata(DecoratorType.UNUPDATABLE, target) || []
        unInsertable.push(propertyKey)
...
export function ManyToMany(type: unknown, table: string) {
    return function (target: Model, propertyKey: string) {
        ...
        const m2m =
            Reflect.getMetadata(DecoratorType.MANY_TO_MANY, target) || []
