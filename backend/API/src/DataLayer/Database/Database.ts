...
export class Database {
    ...
    private static db: Connection = mysql.createConnection(config.default.Database);
    tableKeys: IDictionary<string> = JSON.parse(JSON.stringify(dbKeys.default));
    ...
    dbSelectAll(tableName: string): Promise<DatabaseResponse> {
        return new Promise((resolve, reject) => {
            ...
            Database.db.query(command, (err, results) => {
                if (err) {
    ...
    dbPost(body: IDictionary<any>, tableName: string): Promise<DatabaseResponse> {
        return new Promise((resolve, reject) => {
            ...
            Database.db.query<ResultSetHeader>(command, data, (err, results) => {
                if (err) {
                    console.error('Error querying the database:', err);
                    ApiLogger.logApi(err.toString());
                    reject(new DatabaseFail(err));
